...
function FloatingParticles() {
  ...
  useEffect(() => {
    // Reduce particles on small screens to avoid heavy paints on mobile
    const isSmall = typeof window !== 'undefined' && window.innerWidth < 768;
    const count = isSmall ? 12 : 80;
    const newParticles = [];
    for (let i = 0; i < count; i++) {
      const isBlue = Math.random() > 0.5;
      const size = Math.random() * (isSmall ? 8 : 12) + (isSmall ? 4 : 8); // smaller on mobile
      newParticles.push({
        id: i,
        x: Math.random() * 100,
        y: Math.random() * 100,
        size: size,
        duration: Math.random() * (isSmall ? 6 : 8) + (isSmall ? 4 : 6),
        delay: Math.random() * (isSmall ? 2 : 5),
        color: isBlue ? '51, 102, 204' : '255, 51, 51',
        moveX: (Math.random() - 0.5) * (isSmall ? 40 : 100),
        moveY: (Math.random() - 0.5) * (isSmall ? 40 : 100),
      });
    }
...
export default function About() {
  ...
  useEffect(() => {
    const check = () => {
      ...
      setIsMobile(small);
      if (small) document.body.classList.add('about-hide-overlays');
      else document.body.classList.remove('about-hide-overlays');
    };
    check();
    window.addEventListener('resize', check);
    return () => {
      window.removeEventListener('resize', check);
      document.body.classList.remove('about-hide-overlays');
    };
  }, []);
