...
function GalaxyBackground({ isMobile }) {
  const [scrollY, setScrollY] = useState(0);
  const [stars, setStars] = useState([]);

  useEffect(() => {
    if (isMobile) return; // skip scroll listener on mobile to reduce paints
    const handleScroll = () => {
  ...
  useEffect(() => {
    if (isMobile) return; // do not generate animated stars on mobile
    const newStars = [];
    for (let i = 0; i < 200; i++) {
      newStars.push({
        id: i,
  ...
  if (isMobile) {
    // Simplified static background for mobile: keep image but remove animated overlays
    return (
...
export default function StrangerThingsContact() {
  ...
  const reviews = [
    {
      title: "Fandom Frenzy Quiz Was A Blast",
      rating: 5,
      text: "Fandom Frenzy Quiz was fast-paced and incredibly fun. The rounds tested our knowledge across movies, TV shows, and comics with buzzer challenges that kept the adrenaline high. The questions were well-balanced—tough but fair—and the hosts kept the energy up. Our team loved the variety and the quick rounds encouraged quick thinking and great teamwork.",
